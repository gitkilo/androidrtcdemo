...
private String peerId = "";
  public void makeRequest() {
    ...
    final String reqUrl = roomUrl + String.format("/sign_in?%s", "android");
    AsyncHttpURLConnection httpConnection =
        new AsyncHttpURLConnection("GET", reqUrl, roomMessage, new AsyncHttpEvents() {
          @Override
  ...
  {
    final String reqUrl = roomUrl + String.format("/wait?peer_id=%s", peerId);
    AsyncHttpURLConnection httpConnection =
            new AsyncHttpURLConnection("GET", reqUrl, roomMessage, new AsyncHttpEvents() {
              @Override
              public void onHttpError(String errorMessage) {
...
//                int peerId = roomHttpResponseParse(response);
  ...
  private void readlyToVideo(String response)
  {
    Log.d(TAG, "readlyToVideo:" + response);
    try {
      ...
      if (json.has("type"))
      {
        ...
        {
          events.onSignalingParametersError("not get type offer.");
          return;
        }
        if (json.has("sdp"))
        {
          String sdp = json.getString("sdp");
          if (null == sdp || "".equals(sdp.trim()))
          {
            events.onSignalingParametersError("sdp is empty.");
            return;
          }
          SessionDescription offerSdp = new SessionDescription(
                  SessionDescription.Type.fromCanonicalForm(type), sdp);
          SignalingParameters params = new SignalingParameters(
                  new ArrayList<>(), true, peerId, "", "", offerSdp, new ArrayList<>());
  ...
  private String roomHttpResponseParse(String response)
  {
      ...
      String peerId = resp[1];
      return peerId;
  }

  private void roomHttpResponseParse1(String response) {
    Log.d(TAG, "Room response: " + response);
    try {
